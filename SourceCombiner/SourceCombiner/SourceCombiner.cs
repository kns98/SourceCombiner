    ...
    private static void Main(string[] args)
        {
            ...
            if (args.Length > 2) bool.TryParse(args[2], out openFile);

            if (args.Length > 3) bool.TryParse(args[3], out minify);
            ...
            if (openFile) Process.Start(outputFilePath);
        }
        ...
        private static string GenerateCombinedSource(List<string> namespaces, List<string> files)
        {
            ...
            foreach (var ns in namespaces.OrderBy(s => s)) sb.AppendLine("using " + ns + ";");
            ...
            foreach (var file in files)
            {
                ...
                foreach (var sourceLine in sourceLines)
                {
                    ...
                    var isUsingDirective =
                        trimmedLine.StartsWith(usingDirectiveStartsWith) && trimmedLine.EndsWith(";");
                    if (!string.IsNullOrWhiteSpace(sourceLine) && !isUsingDirective) sb.AppendLine(sourceLine);
                }
            }
        ...
        private static List<string> GetUniqueNamespaces(List<string> files)
        {
            ...
            foreach (var file in files)
            {
                ...
                foreach (var sourceLine in sourceLines)
                {
                    ...
                    {
                        var name = trimmedLine.Substring(namespaceStartIndex,
                            trimmedLine.Length - namespaceStartIndex - 1);
